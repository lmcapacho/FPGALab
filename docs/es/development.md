...
El renderizador `measured_svg` permite definir la apariencia en el paquete: `base_svg` queda fijo y `moving_svg` gira (`rotate`) o cambia de escala horizontal (`scale_x`) según `duty_cycle` o `pulse_high_seconds`. El manifiesto fija terminal, rango de entrada y salida y origen de la transformación. El ejemplo `pulse_servo` usa ahora `body.svg` y `arm.svg`; no necesita un dibujo específico en Python. El renderizador anterior `pulse_servo` se conserva para paquetes existentes.

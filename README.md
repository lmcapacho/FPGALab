...
FPGALab resolves Verilator in this order: the toolchain installed by Apio/Icestudio, a standalone OSS CAD Suite configured with `FPGALAB_OSS_CAD_SUITE`, and finally `verilator` on the system `PATH`. An explicit `FPGALAB_VERILATOR` setting overrides automatic discovery.
